...
class ProdutoADMApp:
    def __init__(self, root):
        ...
        self.root.configure(background="#e6f2ff")  # azul claro/quase branco
        self.criar_menu()
        self.criar_widgets()

    def criar_menu(self):
        menu_bar = Menu(self.root)
        menu_cadastros = Menu(menu_bar, tearoff=0)
        menu_cadastros.add_command(label="Produtos", command=lambda: None)
        menu_cadastros.add_command(label="Funcionários", command=lambda: None)
        menu_cadastros.add_command(label="Fornecedores", command=lambda: None)
        menu_cadastros.add_command(label="Clientes", command=lambda: None)
        menu_bar.add_cascade(label="Menu", menu=menu_cadastros)
    ...
    def adicionar(self):
        try:
            dados = self.obter_dados_entrada(exceto_id=True)
            add_product(**dados)
            self.limpar_campos()
    ...
    def listar(self):
        try:
            ...
            for p in produtos:
                self.text_area.insert(tk.END, f"ID: {p[0]} | Nome: {p[1]} | Tipo: {p[2]} | Quantidade Enviada: {p[3]} | Validade: {p[4]} | Fabricação: {p[5]} | Lote: {p[6]} | Fornecedor: {p[7]} | Estoque: {p[8]}\n")
        except Exception as e:
            messagebox.showerror("Erro", str(e))

    def obter_dados_entrada(self, exceto_id=False):
